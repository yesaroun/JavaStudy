...
○ 인터페이스(Interface)란
	...
	몸체(정의부)가 없기 때문에 사실상 실행 부분이 존지하지 않는다.
...
○	인터페이스는 클래스와 달리 다중 상속이 가능하며
	...
	인터페이스를 통해 다중 상속을 구현하는 방법을 지원힌다.

○	인터페이스는 상수와 추상 메소드만 가질 수 있으며
	인터페이스 안의 메소드들은 접근제어지시자를 명시하지 않아도
	『public』(많이 사용해야하니까)으로 설정되어 클래스에서 구현(implements)함으로써
	바로 접근이 이루어질 수 있다.
...
○ 특징
	...
	- 인터페이스를 implements 한 클래스는
	  인터페이스의 모든 메소드를 Overrriding 해야 한다.
...
public class Test119
{
	public static void main(String[] args)
	{
		...
		System.out.println(Demo.a);				//-- 자동으로 static이 붙기 때문에
		//--==>> 10
