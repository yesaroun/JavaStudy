...
public class Test142
{
	public static void main(String[] args) throws IOException
	{
		...
		n=0;	
		while (st.hasMoreTokens())		// 다음 값이 있으면 true반환/ 없으면 false
		{
			strArr[n++] = st.nextToken(); // 그러면 사과 얻어 오는거고 그 다음 얻어오고 그럼
			// strArr[0] = "사과";
			// strArr[1] = "딸기";
			//		:
			// strArr[3] = "토마토";
		...
		//	   확인 결과만 반환할 뿐
		//	   실제 내용을 읽어들이지도 않고
		//	   실제 내용을 읽어들이는 해드를 이동시키지도 않는다.
		//-- 『nextToken()』
		//	 :  다음 토큰을 얻어온다.
		//		다음 토큰을 얻어오며 해드를 이동시키게 된다.
		//		구분자를 바꾸었을 경우 다음 호출에도
		//		바뀐 구분자를 이용하여 토큰을 분리한다.

		System.out.print("파생될 토큰(문자열) : ");
