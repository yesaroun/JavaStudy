...
■ 캐릭터 셋(Character Set)
	○ 유니코드(unicode)
	   국제표준으로 제정된 2바이트계의 만국 공통 국제 문자부호
	   ...
	   공식 명칭은 ISO/IEC 10646-1(Universal Multiple-Octer Coded Character Set)이다.

	   데이터의 교환을 원활하게 하기 위하여 문자 1개에 부여되는 값을
	   16비트로 통일하였다.
	   코드의 1문자 당 영어는 7비트, 비영어는 8비트
	   한굴이나 일본어는 16비트의 값을 가지게 되는데
	...
	- UTF-16(16bit UCS Transformation Format)
	  자바의 기본코드. 자바에서는 문자 하나를 저장하며 바이트 수는
...
■ 문자 코드의 종류
	...
	○ KSC5636
	   ...
	   기본 ASCII Code에서 역슬래시(\)를 (￦) 표시로 대체

	○ EUC-KR(MS949)
	   Bell-Laboratories 에서 유닉스 상에서 영문자 이외의 문자를
	   지원하기 위해서 제안한 확장 유니코드(Extend UNIX Code) 중
	   한글 인코딩 방식
	   영문은 K5C5636으로 처리하고, 한글은 KSC5601로 처리
	   즉, EUC-KR(MS949) = K5C5601

	○ UTF-8
	   영문과 숫자는 1바이트, 한글은 3바이트를 표현
