...
※ RMI(Remote Method Invocation)
	...
	통신 구조를 지원하는 개념을 이해하면 좋겠다.

※ Bean(빈)
	C/S 구조적 모델에서 서버 측 구조에 해당하며,
...
○ Serializable 인터페이스
	...
	형식)
	1
	public class 클래스명 implemnets Serializable
	{
		// ...
	}
	...
	ObjectInputStream 클래스와 ObjectOutputStream 클래스를 이요하여
	객체 단위로 입출력을 수행하게 된다.
...
public class Test179
{
	...
	{
		String appDir = System.getProperty("user.dir");	// 유저 사용정보를 얻어옴
		...
		// 파일 객체 생성
		File f0 = new File(appDir, "\\data\\test.ser");			// \이거 두번해야 인식함
		//-- appDir → C:\JavaStudy
		...
		// 테스트 2
		// 『C:\JavaStudy』 경로에 『data』 디렉토리 생성 후 다시 확인
		System.out.println(f0.getParentFile().exists());
		//--==>> true
		//-- 『test.ser』 파일이 만들어질 디렉토리 경로가 구성되어 있다.
		...
		//-- 파일 전용출력 스트림(물줄기)에
		//	 f0 라는 파일 객체를 띄우겠다...
		//InputStreamReader isr = new InputStreamReader(System.in);//이거랑 같다
		...
		if(f0.exists())
		{
			...
			Hashtable h2 = (Hashtable)ois.readObject();			// Object 객체를 다운캐스팅하는거임
